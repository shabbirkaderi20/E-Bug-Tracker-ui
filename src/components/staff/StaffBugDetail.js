...
function RenderBug({bug}) {
        ...
        let tagStaffDto= {
            ...
            let headers = {
              ...
              if(window.confirm("Are you sure ?")){
                axios.post("http://localhost:9004/bugs/transferBug", tagStaffDto, headers)
                 .then(response=> {
                     if(response.data) {
    ...
    const solveBug= (bugId) => {

        if(window.confirm("Are you sure ?")) {

            axios.get(`http://localhost:9004/bugs/solveBug/${bugId}`) 
            .then(response=> {
                if(response.data) {
                    alert("Bug solved successfully");
                   window.location.reload(true);
                }
               })
               .catch(error=> {
    ...
    return (<div className='col-12 col-md-5 m-1'>
        ...
        {!bug.bugSolved && bug.status==="assigned"?<Button className= "btn btn-primary-outline btn-rounded btn-block" online onClick= {()=> {
                handleSubmit2();
                     solveBug(bug.bugId);}}>
                        Solve Bugs
        </Button>: <></>}
...
function GetStamp(postId) {
    ...
    }else if(postId=== parseInt(sessionStorage.getItem("userId"))) {
